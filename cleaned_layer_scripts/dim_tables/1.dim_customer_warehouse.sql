...
IF OBJECT_ID('pc_sales_dw.dbo.dim_customer', 'U') IS NOT NULL BEGIN
/* Drop the existing table to recreate it fresh */
DROP TABLE
  pc_sales_dw.dbo.dim_customer;
...
SELECT
  ...
  TRIM(ISNULL(Customer_Email_Address, '')) AS Customer_Email_Address -- FROM: Source table in staging database
FROM
  pc_sales_stg.dbo.dim_customer -- WHERE: Only include records where all required fields have actual content
  -- LEN(): Check the length after trimming
WHERE
  LEN(TRIM(ISNULL(Customer_Name, ''))) > 0 -- Name must not be empty
